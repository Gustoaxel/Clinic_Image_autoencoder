...
class CAE(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            rand = torch.rand(ec3.shape, device=torch.device('cuda'))
            prob = (1 + ec3) / 2
            eps = torch.zeros(ec3.shape, device=torch.device('cuda'))
            eps[rand <= prob] = (1 - ec3)[rand <= prob]
            eps[rand > prob] = (-ec3 - 1)[rand > prob]

        # encoded tensor
        self.encoded = 0.5 * (ec3 + eps + 1)  # (-1|1) -> (0|1)
        encode = torch.flatten(self.encoded, 1)
        #encode = self.classifier(encode)

        return self.classifier(encode), self.decode(self.encoded)
